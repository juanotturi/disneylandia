    ...
    public Specification<PersonajeEntity> getByFilters(PersonajeFiltersDto filtersDto) {
        return ((root, query, criteriaBuilder) -> {
            ...
            if (StringUtils.hasLength(String.valueOf(filtersDto.getEdad()))) {
                predicates.add(criteriaBuilder.equal(root.get("edad"), Long.toString(filtersDto.getEdad())));
            }
            if (StringUtils.hasLength(String.valueOf(filtersDto.getPeso()))) {
                predicates.add(criteriaBuilder.equal(root.get("peso"), Long.toString(filtersDto.getPeso())));
            }
